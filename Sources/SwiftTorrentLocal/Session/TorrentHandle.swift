...
public actor TorrentHandle {
    ...
    private var announceAttempts: Int = 0
    ...
    public init(params: AddTorrentParams, settings: SessionSettings, group: EventLoopGroup) {
        ...
        if let magnet = params.magnetLink, !magnet.trackers.isEmpty {
            ...
            print("DEBUG: TorrentHandle init with \(magnet.trackers.count) trackers")
        } else {
            print("DEBUG: TorrentHandle init with NO trackers (magnet: \(params.magnetLink != nil))")
        }
    }
    ...
    private func setupDownloadComponents(info: TorrentInfo) async {
        ...
        self.piecePicker = pp
        self.diskIO = dio
        print("DEBUG: Download components setup. Pieces: \(info.pieceCount), Total Size: \(info.totalSize)")

        if self.trackerManager == nil {
            self.trackerManager = TrackerManager(info: info, group: group)
            print("DEBUG: TrackerManager created from TorrentInfo")
        }
    ...
    public func start() async throws {
        ...
        print("DEBUG: TorrentHandle.start() called. State: \(state)")
        ...
        if let trackerMgr = trackerManager {
            let totalSize = info?.totalSize ?? 1024 * 1024 * 100 // Default to 100MB if unknown (magnet) to avoid claiming Seeder status (left=0)
            let left = totalSize - totalDownloaded
            let params = AnnounceParams(
                infoHash: infoHash, peerID: peerID, port: 50000,
                left: left, event: "started"
            )
            print("DEBUG: Announcing to trackers... (Left: \(left))")
            await announceToAllTrackers(trackerMgr: trackerMgr, params: params)
            startReannounceLoop(trackerMgr: trackerMgr)
        } else {
             print("DEBUG: No TrackerManager in start()")
        }
    }
    ...
    private func announceToAllTrackers(trackerMgr: TrackerManager, params: AnnounceParams) async {
        do {
            let response = try await trackerMgr.announce(params: params)
            for (address, port) in response.peers {
                await peerManager.addPeer(address: address, port: port)
            }
        } catch {
             print("Tracker announce failed: \(error)")
        }
    }
    ...
    private func startReannounceLoop(trackerMgr: TrackerManager) {
        reannounceTask = Task { [weak self] in
            while !Task.isCancelled {
                ...
                // Exponential backoff when no connected peers: 30s, 60s, 120s, 300s (max)
                let peerCount = await self.peerManager.connectedCount
                let baseInterval = await trackerMgr.getInterval()
                
                let interval: Int
                if peerCount == 0 {
                    let attempts = await self.announceAttempts
                    // Exponential backoff: 30, 60, 120, 300, 300, ...
                    let backoffIntervals = [30, 60, 120, 300]
                    let backoffIndex = min(attempts, backoffIntervals.count - 1)
                    interval = backoffIntervals[backoffIndex]
                    await self.incrementAnnounceAttempts()
                } else {
                    // Have connected peers, use normal interval and reset attempts
                    interval = max(baseInterval, 60)
                    await self.resetAnnounceAttempts()
                }
                
                print("DEBUG: Re-announce loop. Connected peers: \(peerCount). Attempt: \(await self.announceAttempts). Next in \(interval)s")
                ...
                let totalSize = await self.info?.totalSize ?? 1024 * 1024 * 100
                let left = totalSize - downloaded
                let params = AnnounceParams(
                    infoHash: infoHash, peerID: peerID, port: 50000,
                ...
                do {
                    ...
                    print("Tracker re-announce failed: \(error)")
                }
            }
        }
    }
    ...
    private func incrementAnnounceAttempts() {
        announceAttempts += 1
    }
    
    private func resetAnnounceAttempts() {
        announceAttempts = 0
    }
}
