...
public final class PeerConnection: @unchecked Sendable {
    ...
    public func connect(on group: EventLoopGroup) async throws -> Channel {
        do {
            ...
            if let remoteID = decoder.remotePeerID {
                 print("DEBUG: Handshake success with \(address):\(port). PeerID: \(String(decoding: remoteID, as: UTF8.self))")
            }

            return ch
        } catch {
            print("DEBUG: Connection to \(address):\(port) failed: \(error)")
            throw error
        }
    }
