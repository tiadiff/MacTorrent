...
public final class UDPTracker: Sendable {
    ...
    public func announce(params: AnnounceParams) async throws -> AnnounceResponse {
        ...
        let needsResolution = host.rangeOfCharacter(from: CharacterSet(charactersIn: "0123456789.:").inverted) != nil
        if needsResolution {
            // It's a hostname, resolve it
            do {
                resolvedHost = try await resolveHostname(host)
            } catch {
                print("DEBUG: Failed to resolve host \(host): \(error)")
                throw error
            }
        } else {
            resolvedHost = host
        ...
        try await withCheckedThrowingContinuation { continuation in
            DispatchQueue.global().async {
                ...
                guard status == 0, let addrInfo = result else {
                    continuation.resume(throwing: TrackerError.dnsFailed)
                    return
                }
                ...
                let len = hostBuf.firstIndex(of: 0) ?? hostBuf.count
                let hostString = String(decoding: hostBuf[..<len].map(UInt8.init(bitPattern:)), as: UTF8.self)
...
private final class UDPResponseHandler: ChannelInboundHandler, @unchecked Sendable {
    ...
    func waitForResponse(timeout: TimeAmount) async throws -> Data {
        let existingData = lock.withLock { () -> Data? in
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let (dataFound, idToWait) = lock.withLock { () -> (Data?, UInt64?) in
                if !receivedData.isEmpty {
                    return (receivedData.removeFirst(), nil)
                } else {
                    let id = nextID
                    nextID += 1
            ...
            if let id = idToWait {
                Task {
                    ...
                    cont?.resume(throwing: TrackerError.timeout)
                }
            }
        }
    }
